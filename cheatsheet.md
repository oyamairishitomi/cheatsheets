...
php artisan config:clear                       # .envを変更したあとに実行（キャッシュをクリア）
```
...
### 取得結果を変数に代入しないと消える
```php
Term::where('user_id', Auth::id())->get(); // ❌ 取ってきたデータが消える

$terms = Term::where('user_id', Auth::id())->get(); // ✅ $termsに保存
return view('dashboard', compact('terms'));           // bladeへ渡せる
```
メソッドは値を**返す**だけ。使いたければ変数に受け取る。
...
public function index(){
    ...
    return view('dashboard', compact('terms'));
}
```
...
public function destroy(Request $request, $id){
    $term = Term::findOrFail($id); // 存在しなければ404
    if (auth()->id() !== $term->user_id) abort(403); // 他人のデータは403
    $term->delete();
...
<form action="{{ url('/delete') }}/{{ $term->id }}" method="POST">
  @method('DELETE')  {{-- DELETEメソッドはフォームで直接使えないのでLaravelが変換 --}}
  @csrf
...
```javascript
fetch('{{ url("/送り先") }}', {   // Bladeの中ではurl()でベースURLを自動補完
  method: 'POST',
  headers: {
    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
    // FormDataを送るときはContent-Typeを書かない（ブラウザが自動セット）
    // JSONを送るときは 'Content-Type': 'application/json' を追加
  },
  body: formData,             // ファイル送信の場合
  // body: JSON.stringify({key: value}), // JSON送信の場合
})
.then(res => res.json())
...
start.addEventListener('click', async () => {
  const stream = await navigator.mediaDevices.getUserMedia({ audio: true }); // マイク許可
  mediaRecorder = new MediaRecorder(stream);
  audioChunks = [];

  mediaRecorder.ondataavailable = (event) => { // 録音中、チャンクが届くたびに発火
    if (event.data.size > 0) audioChunks.push(event.data);
  };

  mediaRecorder.onstop = () => {               // stop()後に発火。ここでfetchを送る
    const audioBlob = new Blob(audioChunks, { type: 'audio/webm' });
    const formData = new FormData();
    formData.append('audio', audioBlob, 'recording.webm'); // Controllerの$request->file('audio')と対応
    // fetchでサーバーへ送信...
  };
...
stop.addEventListener('click', () => {
  if (mediaRecorder && mediaRecorder.state !== 'inactive') { // 存在して録音中のときだけ止める
    mediaRecorder.stop(); // stop()を呼ぶとonstopが発火する
  }
});
```

### mediaRecorder.state の値
| state | 意味 |
|---|---|
| `'recording'` | 録音中 |
| `'paused'` | 一時停止中 |
